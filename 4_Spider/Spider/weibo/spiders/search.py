...
class SearchSpider(scrapy.Spider):
    ...
    def get_location(self, selector):
        ...
        #location_div = selector.xpath('//div[starts-with(@title, "发布于 ") and @class="head-info_ip_3ywCW"]')

        #if location_div:
             # 获取<title>属性中的位置信息
        #    location = location_div[0].get('title').replace("发布于 ", "")
        #    return location
        #else:
        #    return "Location information not found."
        a_list = selector.xpath('.//div')
        location = ''
        for a in a_list:
            if a.xpath('//div[starts-with(@title, "发布于 ") and @class="head-info_ip_3ywCW"]'):
                location = a.xpath('@title').extract_first().replace("发布于 ", "")
    ...
    def parse_weibo(self, response):
        ...
        for sel in response.xpath("//div[@class='card-wrap']"):
            ...
            if info:
                ...
                location_sel = sel.xpath('//div[starts-with(@title, "发布于 ") and @class="head-info_ip_3ywCW"]')
                ...
                if content_full:
                    if not retweet_sel:
                        ...
                        txt_sel = content_full[0]
                        is_long_weibo = True
                #weibo['location'] = self.get_location(location_sel)
                #if weibo['location']:
                ...
                dt = datetime.strptime(f"{year}年{created_at}", "%Y年%m月%d日%H:%M")
                timestamp = time.mktime(dt.timetuple())
                weibo['created_at'] = timestamp
