...
class CsvPipeline(object):
    def process_item(self, item, spider):
        # 从 settings 中获取自定义的输出目录
        settings = get_project_settings()
        base_dir = os.path.join(settings.get('RESULTS_DIR', '结果文件'), item['keyword'])
        ...
        # 定义输出文件路径
        file_path = os.path.join(base_dir, f"{item['keyword']}_output.csv")
        ...
        if item:
            with open(file_path, 'a', encoding='utf-8-sig', newline='') as f:
                ...
                #if is_first_write:
                    #header = [
                    #    'user_id', '发布位置', '微博id', '发布时间', '点赞数', '评论数', '微博正文',
                   # ]
                    #header = [
